...
parser.add_argument('--epochs_scheduler', default=100, type=int, help='Total epochs for scheduler')
    ...
    def __init__(
        ...
        for _ in range(n_layers):
            self.s4_layers.append(
                HOPE_SSM(d_model, dropout=dropout, transposed=True, lr=min(args.lr_min, args.lr), dt_min=args.min_dt, dt_max=args.max_dt, lr_dt=args.lr_dt, wd=args.wd)
            )
...
optimizer, scheduler = setup_optimizer(
    model, lr=args.lr, weight_decay=args.weight_decay, epochs=args.epochs_scheduler, warmup_epochs=args.warmup
)
